...
public class AreaOfQuadrangle {
	...
	public static double calculateAnyProtuberant (double length1, 
												  double length2, 
												  double length3, 
												  double length4) {
		...
		double area;
		area = StrictMath.sqrt((half_perimeter - length1) 
			   * (half_perimeter - length2) * (half_perimeter - length3) 
			   * (half_perimeter - length4) - length1 * length2 * length3 
			   * length4 * ((1 + StrictMath.cos(2 * teta)) / 2));
		return area;

	}
	
	public static double calculateConcavedDeltoid (double length1, 
												   double length2, 
												   double length4) {
		
		double area = length1 * length2 
					  * StrictMath.sin(StrictMath.atan(length2 / length4));
		return area;
	}
}
